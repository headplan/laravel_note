...
# 数据库配置
    ...
    配置文件里没有这两种数据库的配置
## SQLite 配置
    使用touch database/database.sqlite命令创建
...
$pdo = DB::connection()->getPdo(); // 访问底层PDO

2.运行原生SQL查询
    $users = DB::select('select * from users where active = ?', [1]);
    DB::insert('insert into users (id, name) values (?, ?)', [1, 'Dayle']);
    $affected = DB::update('update users set votes = 100 where name = ?', ['John']); // 返回行数
    $deleted = DB::delete('delete from users'); // 返回行数
    DB::statement('drop table users'); // 通用在不返回内容的SQL
    ## 命名绑定
    $results = DB::select('select * from users where id = :id', ['id' => 1]);
    
    select方法以数组的形式返回结果集,数组中的每一个结果都是一个PHP StdClass对象,可以这样操作
...
3.数据库事务
    DB::transaction闭包抛出异常自动回滚,执行成功自动提交.
    DB::transaction(function () {
        DB::table('users')->update(['votes' => 1]);
        DB::table('posts')->delete();
    });
# 处理死锁
    DB::transaction闭包第二个参数,用于定义死锁发生时事务的最大重试次数.
    DB::transaction(function () {
        DB::table('users')->update(['votes' => 1]);
        DB::table('posts')->delete();
    }, 5);
...
※ 使用DB门面的事务方法还可以用于控制[查询构建器]和[Eloquent ORM]的事务
```
...
在一个数据库事务中运行一连串操作,可以使用DB门面的transaction方法.如果事务闭包中抛出异常,事务将会自动回滚.执行成功了,事务将会自动提交.

使用`transaction`方法时不需要担心手动回滚或提交:
...
如果想手动开始事务,从而对回滚和提交有一个完整的控制,可以使用DB门面的beginTransaction方法:
...
> 注:使用DB门面的事务方法还可以用于控制查询构建器和Eloquent ORM的事务.
