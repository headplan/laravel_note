...
然后是添加Cookie到响应头 . 过程是先判断session是否是持久化存储\(不是数组驱动的就是持久化的\) , 持久化的情况下会将SessionID设置到响应头 , 就是在响应实例中的$headers属性中添加一个Cookie实例 , 用于发送SessionID . 最后就是session的存储 , 通过terminate\(\)函数 . 
