...
Eloquent的all方法返回模型表的所有数据.由于每一个Eloquent模型都是一个查询构建器,所以还可以添加约束条件到查询,然后用get方法获取对应的结果
...
> 注:由于Eloquent模型本质上就是查询构建器,可以在查询中会用查询构建器的所有方法

**集合**

对Eloquent中获取多个结果的方法\(比如all和get\)而言,其返回值是`Illuminate\Database\Eloquent\Collection`的一个实例,Collection类提供了多个有用的函数来处理Eloquent结果.

> 当然,也可以像前面,把实例结果集当成简单的数组循环

**组块结果集**

如果需要处理成千上万个结果,内存消耗一定很大.可以使用chunk方法获取一个组块的Eloquent模型,并将其填充到给定的闭包进行处理
...
除了从给定表中获取所有记录之外,还可以使用find和first获取单个记录.这些方法返回单个模型实例而不是返回模型集合
...
有时候可能想要在模型找不到的时候抛出异常,这在路由和控制器中非常有用,findOrFail和firstOrFail方法会获取到查询到的第一个结果.如果没有任何结果,`Illuminate\Database\Eloquent\ModelNotFoundException`异常将会被抛出

```
$model = App\Flight::findOrFail(1);
$model = App\Flight::where('active', 1)->first();
```

如果异常没有被捕获,那么HTTP 404响应将会被发送给用户,所以在使用这些方法的时候没有必要对返回404响应编写明确的检查:
...
还可以使用查询构造器聚合方法,例如count,max,sum,以及其他查询构建器提供的聚合方法.
