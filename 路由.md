...
```
1. 基本路由
   1. 默认文件 - Route::get\(\);新版本的路由可分文件了
   2. 有效的路由文件 - get,post,put,patch,delete,options.多个请求可以使用match\(后面接数组参数\)或any方法
   3. CSRF防护 - CSRF令牌字段,模板页面应用{{ csrf\_field\(\) }}
2. 路由参数
   1. 必选参数{id}
   2. 可选参数{name?}
   3. 正则约束 - -&gt;where\(\),参数可以是逗号或数组
   4. 全局约束 - 在路由的服务器提供者RouteServiceProvider的boot中添加Route::pattern\('id', '\[0-9\]+'\);
3. 路由命名 - 路由命名分两步完成
   1. 首先,在路由中-&gt;name\('abc'\);
   2. 之后就可以用route\('abc'\)函数生成路由url了,函数第二个参数可以指定路由参数
4. 路由群组
   1. Route:group\(\);共享路由属性
   2. 中间件 - \['middleware' =&gt;'auth'\]
   3. 命名空间 - \['namespace' =&gt;'Admin'\],默认RouteServiceProvider模板包含了App\Http\Controllers
   4. 子域名路由 - \['domain' =&gt;'{account}.myapp.com'\]
   5. 路由前缀 - \['prefix' =&gt;'admin'\]
5. 路由模型绑定
   1. 隐式绑定
      默认传入id返回内容
   ...
   2. 显示绑定
      在 RouteServiceProvider 类的 boot 方法中定义模型绑定
      public function boot()
      {
          parent::boot();
          Route::model('user', App\User::class);
      }
      把model方法换成bind自定义绑定逻辑
6. 表单方法伪造
...
由于已经绑定 `{user}` 参数到 `App\User` 模型,User 实例会被注入到该路由.因此,如果请求 URL 是`profile/1`,就会注入一个用户 ID 为 1 的 User 实例.如果匹配的模型实例在数据库不存在,会自动生成并返回 HTTP 404 响应.

自定义解析逻辑

如果需要使用自定义的解析逻辑,需要使用`Route::bind`方法,传递`bind`方法的闭包会获取到URI请求参数中的值,并且返回需要的在该路由中注入的类实例:
